...
class CKCall {
  ...
  String? difference(CKCall call) {
    ...
    if (uuid != call.uuid) changes.add('uuid');
    if (localizedName != call.localizedName) changes.add('localizedName');
    if (dateStarted != call.dateStarted) changes.add('dateStarted');
    if (dateUpdated != call.dateUpdated) changes.add('dateUpdated');
    if (attributes != call.attributes) changes.add('attributes');
    if (callType != call.callType) changes.add('callType');
    if (state != call.state) changes.add('state');
    if (data != call.data) changes.add('data');
    if (capabilities != call.capabilities) changes.add('capabilities');
    return changes.join(', ');
  }
