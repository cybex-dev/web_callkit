...
class CKSound {
  ...
  const CKSound({required this.type, this.looping = false, this.enabled = true, this.delay = const Duration(seconds: 0)});
}

class CKURLSound extends CKSound {
  final String url;

  const CKURLSound(this.url, {super.looping, super.enabled, super.delay, super.type = CKSoundType.url});
}

class CKAssetSound extends CKSound {
  final String asset;

  const CKAssetSound(this.asset, {super.looping, super.enabled, super.delay, super.type = CKSoundType.asset});
}
